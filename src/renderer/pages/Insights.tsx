...
import { generateInsights, computeInsightStats, GeneratedInsight } from '../lib/insightEngine'
...
interface StoredInsight {
  id: number; userId: number; habitId: number | null
  type: string; content: string; generatedAt: string; isRead: boolean
}

// ── Components ────────────────────────────────────────────────
function InsightCard({ insight, habitName, onDismiss }: {
  insight: StoredInsight; habitName: string | null; onDismiss: (id: number) => void
}) {
  ...
  return (
    <div
      ...
      style={{ position: 'relative', background: insight.isRead ? 'var(--surface-soft)' : 'var(--canvas)', border: `1px solid ${hovered && !insight.isRead ? cfg.color + '50' : 'var(--hairline)'}`, borderRadius: 'var(--radius-xl)', padding: '20px 22px', transition: 'all var(--transition-fast)', boxShadow: hovered && !insight.isRead ? `0 4px 20px ${cfg.color}12` : 'var(--shadow-sm)', opacity: insight.isRead ? 0.7 : 1, overflow: 'hidden' }}
    >
      {!insight.isRead && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${cfg.color}, transparent)`, borderRadius: 'var(--radius-xl) var(--radius-xl) 0 0' }} />
      )}
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
        <div style={{ width: 42, height: 42, borderRadius: 'var(--radius-lg)', background: `${cfg.color}15`, border: `1px solid ${cfg.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: cfg.color, flexShrink: 0, transition: 'transform var(--transition-fast)', transform: hovered && !insight.isRead ? 'scale(1.07)' : 'scale(1)' }}>
          {cfg.icon}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.09em', color: cfg.color, background: `${cfg.color}12`, border: `1px solid ${cfg.color}25`, borderRadius: 'var(--radius-full)', padding: '1px 8px' }}>{cfg.label}</span>
            {habitName && <span style={{ fontSize: 11, color: 'var(--muted)', background: 'var(--surface-card)', borderRadius: 'var(--radius-full)', padding: '1px 8px', border: '1px solid var(--hairline)' }}>{habitName}</span>}
            {!insight.isRead && <span style={{ width: 6, height: 6, borderRadius: '50%', background: cfg.color, flexShrink: 0 }} />}
          </div>
          <p style={{ margin: 0, fontSize: 14, color: insight.isRead ? 'var(--muted)' : 'var(--body)', lineHeight: 1.65 }}>{insight.content}</p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 10, fontSize: 11, color: 'var(--muted-soft)' }}>
            <Clock size={10} />{timeAgo}
          </div>
        </div>
        <button onClick={() => onDismiss(insight.id)} style={{ width: 26, height: 26, borderRadius: 'var(--radius-md)', border: '1px solid var(--hairline)', background: 'transparent', color: 'var(--muted-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, opacity: hovered ? 1 : 0, transition: 'opacity var(--transition-fast)' }} title="Dismiss">
          <X size={11} />
        </button>
      </div>
    </div>
...
function StatsStrip({ habits, logs }: { habits: any[]; logs: any[] }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
      {items.map(item => (
        <div key={item.label} style={{ flex: '1 1 120px', padding: '14px 16px', borderRadius: 'var(--radius-lg)', background: 'var(--canvas)', border: '1px solid var(--hairline)', boxShadow: 'var(--shadow-sm)' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.1rem', color: item.color, letterSpacing: '-0.03em', lineHeight: 1, marginBottom: 5 }}>{item.value}</div>
          <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 500 }}>{item.label}</div>
        </div>
      ))}
      {stats.streakAtRisk && (
        <div style={{ flex: '1 1 120px', padding: '14px 16px', borderRadius: 'var(--radius-lg)', background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.25)' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.1rem', color: '#ef4444', letterSpacing: '-0.03em', lineHeight: 1, marginBottom: 5 }}>⚠️ At risk</div>
...
function GoalsNudge() {
  ...
  return (
    <div style={{ padding: '32px', borderRadius: 'var(--radius-xl)', background: 'var(--canvas)', border: '1px dashed var(--hairline)', textAlign: 'center' }}>
      ...
      <button onClick={() => navigate('/goals')} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 20px', borderRadius: 'var(--radius-md)', background: 'var(--coral)', color: 'var(--on-primary)', fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
...
export default function Insights() {
  ...
  const fetchInsights = async () => {
    ...
    try {
      ...
      list.sort((a: StoredInsight, b: StoredInsight) => new Date(b.generatedAt).getTime() - new Date(a.generatedAt).getTime())
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      for (const insight of generated) {
        const record = await api.insights.create({ userId: user.id, habitId: insight.habitId ?? null, type: insight.type, content: insight.content, isRead: false })
        saved.push(record)
      }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, animation: 'fadeIn 220ms ease forwards' }}>
      ...
      {/* Hero */}
      <div style={{ background: 'var(--surface-dark)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 'var(--radius-xl)', padding: '28px 32px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 7 }}>
              ...
              {unreadCount > 0 && <span style={{ padding: '2px 9px', borderRadius: 'var(--radius-full)', background: 'var(--coral)', color: 'var(--on-primary)', fontSize: 11, fontWeight: 700 }}>{unreadCount} new</span>}
            </div>
          ...
          <button onClick={handleGenerate} disabled={generating || !hasHabits} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '0 24px', height: 46, borderRadius: 'var(--radius-md)', border: 'none', background: generating || !hasHabits ? 'rgba(255,255,255,0.08)' : 'var(--coral)', color: generating || !hasHabits ? 'var(--on-dark-soft)' : 'var(--on-primary)', fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 600, cursor: generating || !hasHabits ? 'not-allowed' : 'pointer', transition: 'all var(--transition-fast)', flexShrink: 0, boxShadow: generating || !hasHabits ? 'none' : '0 2px 12px rgba(204,120,92,0.35)' }}
            ...
            {generating ? (
              <><div style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,0.3)', borderTop: '2px solid white', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />Analysing…</>
            ) : (
              <><Sparkles size={15} />{storedInsights.length === 0 ? 'Generate insights' : 'Refresh insights'}</>
            )}
          </button>
        </div>
      </div>
      ...
      {storedInsights.length > 0 && (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <button onClick={() => setFilterType('all')} style={filterTabStyle(filterType === 'all', 'var(--coral)')}>All ({storedInsights.length})</button>
            {Object.entries(TYPE_CONFIG).map(([key, cfg]) => {
              const count = storedInsights.filter(i => i.type === key).length
              if (count === 0) return null
              return <button key={key} onClick={() => setFilterType(key)} style={filterTabStyle(filterType === key, cfg.color)}>{cfg.label} ({count})</button>
            })}
          </div>
          {loadingInsights ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {filtered.map(insight => <InsightCard key={insight.id} insight={insight} habitName={habitById(insight.habitId)} onDismiss={handleDismiss} />)}
            </div>
          )}
        </>
      )}
      ...
      {storedInsights.length === 0 && hasHabits && !loadingInsights && (
        <div style={{ padding: '48px 32px', borderRadius: 'var(--radius-xl)', background: 'var(--canvas)', border: '1px dashed var(--hairline)', textAlign: 'center' }}>
          ...
          <p style={{ margin: '0 0 24px', fontSize: 13, color: 'var(--muted)', maxWidth: 360, marginLeft: 'auto', marginRight: 'auto' }}>
            Hit "Generate insights" above — Claude will analyse your habits{activeGoals.length > 0 ? ', goals,' : ''} and progress to give you personalised suggestions.
          </p>
          <button onClick={handleGenerate} disabled={generating} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '0 24px', height: 44, borderRadius: 'var(--radius-md)', border: 'none', background: 'var(--coral)', color: 'var(--on-primary)', fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
            <Sparkles size={14} />{generating ? 'Analysing…' : 'Generate now'}
          </button>
        </div>
      )}

      {/* How it works */}
      <div style={{ padding: '22px 24px', borderRadius: 'var(--radius-xl)', background: 'var(--surface-card)', border: '1px solid var(--hairline)', boxShadow: 'var(--shadow-sm)' }}>
