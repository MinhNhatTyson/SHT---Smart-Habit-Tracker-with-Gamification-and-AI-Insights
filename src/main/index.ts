...
async function checkReminders() {
  ...
  for (const habit of habits) {
    ...
    if (Notification.isSupported()) {
      new Notification({ title: `${habit.icon}  Time for: ${habit.name}`, body: "Don't forget your habit — keep that streak alive! 🔥", silent: false }).show()
    }
  }
}
...
ipcMain.handle('store:listItems', async () =>
  prisma.storeItem.findMany({ where: { isActive: true }, orderBy: { sortOrder: 'asc' } })
)

// Get all purchases for a user
ipcMain.handle('store:userPurchases', async (_, userId: number) =>
  prisma.userPurchase.findMany({ where: { userId }, orderBy: { purchasedAt: 'desc' } })
)
